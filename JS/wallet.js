...
let totalCharged = 350;   // matches the demo charge history below
let totalSpent = 100;     // matches the demo purchases table below
...
/* =====================================================
    BALANCE DISPLAY
    Keeps the hero balance card AND both navbar balance
    displays (desktop + mobile) in sync.
===================================================== */
function updateBalanceDisplay() {
    const ids = ['mainBalance', 'studentBalance', 'studentBalanceMobile'];
    ids.forEach(id => {
...
function updateQuickStats() {
    const chargedEl = document.getElementById('totalChargedStat');
    const spentEl = document.getElementById('totalSpentStat');

    if (chargedEl) chargedEl.textContent = `+${totalCharged.toLocaleString('ar-EG')}`;
    if (spentEl) spentEl.textContent = `-${totalSpent.toLocaleString('ar-EG')}`;
}
...
document.addEventListener('DOMContentLoaded', function () {
    const chargeInput = document.getElementById('chargeCodeInput');
    if (chargeInput) {
    ...
    // Make sure quick stats reflect the seeded state on first paint.
    updateQuickStats();
});
